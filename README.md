<div align="center">
<img src="./assets/hero.svg" width="100%" alt="harmeet@github:~$ whoami"/>
</div>
...
Data Scientist Intern at AbbVie's Clinical Innovation R&D team, and a CS junior at UC Irvine who spends most nights on computer vision, LLMs, and the robots that need both to move on their own. Everything below is one terminal session — scroll like it's a log.
...
```yaml
based_in:      Irvine, CA
studying:      B.S. Computer Science (Honors) @ UC Irvine — class of 2028
right_now:     Data Scientist Intern @ AbbVie, Clinical Innovation R&D
off_the_clock:
  - cafe hopping — currently rating every espresso within 5 miles of campus
  - sketching, mostly in the margins of lecture notes
  - pickup basketball, volleyball, and football on weekends
...
```text
[learning]  agentic AI       — multi-agent systems that can actually finish the task, not just chat about it
[learning]  physical AI      — closing the gap between what a model knows and what a body can do with it
[building]  robotics + CV    — systems that perceive first and act second, in that order
[building]  multi-modal LLMs — models that read the room instead of just the prompt
[obsessed]  LLMs             — how far reasoning holds up when you stop babying it
[obsessed]  computer vision  — the original "understand the world" problem, still unsolved
```
...
**[NIMBUS](https://github.com/har-m33t/NIMBUS)** — real-time sign language → English, in the browser
<br/>
Took inference from 800ms to 15ms by compiling an ONNX TGCN model to run client-side in a Web Worker at 30 FPS, then piped it through a serverless AWS backend into Bedrock for live, multi-user captioning under 1.5s end-to-end.
<br/><br/>
`ONNX` `MediaPipe` `AWS Lambda` `Bedrock`

</td>
<td width="50%" valign="top">

**[VIPER](https://github.com/har-m33t/VIPER)** — 🥇 1st place, UCI Datathon 2026 (120+ teams)
<br/>
Fused a fine-tuned CNN with 33 hand-engineered forensic features to catch AI-generated images at 98.9% accuracy across 500K+ images, then made the verdict explainable with Grad-CAM and UMAP instead of a black box.
<br/><br/>
...
**[Six Eyes](https://github.com/har-m33t/six_eyes_drone_intelligence_platform)** — one dashboard, six drones, zero blind spots
<br/>
Streams synchronized live video and telemetry from 6 concurrent UAVs, auto-computes search coverage from operator-drawn map areas, and splits it across the fleet in real time, with on-device YOLOv8n cutting inference load 4x while staying sub-100ms.
<br/><br/>
`Multithreaded Python` `YOLOv8` `React`
